...
WITH ASSIGN_NO AS (
SELECT city_id, day, degree, ROW_NUMBER() OVER(PARTITION BY city_id ORDER BY degree DESC, day) as rn 
FROM weather
)
SELECT city_id, day, degree FROM ASSIGN_NO WHERE rn = 1 ORDER BY city_id