...
Each row of this table shows the time stamps for an employee. in_time is the time the employee started to work, and 
out_time is the time the employee ended work.
...
The number of hours an employee worked can be calculated from the sum of the number of minutes the employee 
worked in all of their sessions. The number of minutes in each session is rounded up.

For example, if the employee worked for 51 minutes and 2 seconds in a session, we consider it 52 minutes.

Write an SQL query to report the IDs of the employees that will be deducted. In other words, report the IDs of the employees 
that did not work the needed hours.
...
WITH FIND_DIFF_MINTS AS (
    SELECT employee_id, SUM(minutes) as total_minutes FROM (
        SELECT employee_id,
        CASE
            WHEN DATEDIFF(second, in_time, out_time)%60 > 0 THEN (DATEDIFF(second, in_time, out_time)/60)+1
            ELSE DATEDIFF(second, in_time, out_time)/60
        END AS minutes
        FROM logs
    ) A 
    GROUP BY employee_id
)
SELECT A.employee_id FROM employees A LEFT JOIN FIND_DIFF_MINTS B ON A.employee_id = B.employee_id
WHERE A.needed_hours*60 > B.total_minutes OR B.total_minutes IS NULL