...
WITH CALCULATE_CONVR_RATE AS (
    SELECT A.member_id, 100.0*SUM(CASE WHEN B.visit_id IS NOT NULL THEN 1 ELSE 0 END)/COUNT(*) AS conversion_rate
    FROM Visits A LEFT JOIN purchases B ON A.visit_id = B.visit_id
    GROUP BY A.member_id
)
SELECT A.member_id, A.name, 
CASE WHEN B.conversion_rate >=80 THEN 'Diamond'
WHEN B.conversion_rate < 80 AND B.conversion_rate >= 50 THEN 'Gold'
WHEN B.conversion_rate < 50 THEN 'Silver'
WHEN B.conversion_rate IS NULL THEN 'Bronze'
END AS category
FROM members A LEFT JOIN CALCULATE_CONVR_RATE B ON A.member_id = B.member_id
