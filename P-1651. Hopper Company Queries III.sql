...
January - March 2020 to October - December 2020. Round average_ride_distance and average_ride_duration to the 
...
Return the result table ordered by month in ascending order, where month is the starting month’s number (January is 1, 
February is 2, etc.).
...
JOINED_DATA AS (
    SELECT MONTH(A.requested_at) AS month_no, B.ride_distance, B.ride_duration
    FROM Rides A LEFT JOIN AcceptedRides B ON A.ride_id = B.ride_id
    WHERE YEAR(requested_at)=2020 
),
AGG_DATA AS (
    SELECT A.month_no, COALESCE(SUM(B.ride_distance), 0) AS ride_distance, COALESCE(SUM(B.ride_duration), 0) AS ride_duration
    FROM GENRATE_MONTH_NO A LEFT JOIN JOINED_DATA B ON A.month_no = B.month_no
    GROUP BY A.month_no
),
FIND_MOVING_AVG AS (
    SELECT month_no, AVG(ride_distance*1.0) OVER(ORDER BY month_no ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING) AS average_ride_distance,
    AVG(ride_duration*1.0) OVER(ORDER BY month_no ROWS BETWEEN CURRENT ROW AND 2 FOLLOWING) AS average_ride_duration
    FROM AGG_DATA
)
SELECT month_NO, ROUND(average_ride_distance, 2) AS average_ride_distance, ROUND(average_ride_duration, 2) AS average_ride_duration
FROM FIND_MOVING_AVG